...
alienBulletPath = (r"./environment/bullet/alienBullet.gif")
alienBulletImage = PhotoImage(file=alienBulletPath)
alienBulletImageItem = canvas.create_image(-200,-200,image=alienBulletImage)

alienBulletPath2 = (r"./environment/bullet/alienBullet2.gif")
alienBulletImage2 = PhotoImage(file=alienBulletPath2)
...
def haveAliensWon():
    global allAlienCO,playerImage,hiddenAliens,gameOver,alienBulletImage,playerCOx,playerCOy,alienBulletImageItem,playerImage
    hiddenAliens=[]
    alienBulletCO = (canvas.coords(alienBulletImageItem))
    for i in range(0,len(allAlienCO)):
        if allAlienCO[i][0] != "Hidden":
    ...
    if ((alienBulletCO[0] - ((1/2)*(playerImage.width()))) <= playerCOx <= (alienBulletCO[0] + ((1/2)*(playerImage.width())))) and ((alienBulletCO[1] - ((1/2)*(playerImage.height()))) <= playerCOy <= (alienBulletCO[1] + ((1/2)*(playerImage.height())))):
        gameOver = True
        return True
...
def alienFire(): ## AAAA
    global allAlienCO,bulletFalling,alienBulletImageItem,alienBulletImage,alienBulletImage2,image,imageCounter,arrayOfAvailableAliens
    arrayOfAvailableAliens = []
    #randomChance = randint(0,300)
    randomChance = 34
    if (randomChance == 34) and (bulletFalling==False):
        bulletFalling=True
        for i in range(0,len(allAlienCO)):
            if allAlienCO[i][0] != "Hidden": #  Finds all non-hidden Alien
                arrayOfAvailableAliens.append(i)

        print(arrayOfAvailableAliens)

        randomAlienNumber = randint(0,len(arrayOfAvailableAliens)-1)

        randomAlienX = allAlienCO[arrayOfAvailableAliens[randomAlienNumber]][1]
        randomAlienY = allAlienCO[arrayOfAvailableAliens[randomAlienNumber]][2]
        
        canvas.coords(alienBulletImageItem, randomAlienX,randomAlienY ) # Where x is the random x co of an alien
                                                                        # Where y is the random y co of an alien + alien image height
        
    if bulletFalling == True:
        canvas.move(alienBulletImageItem,0,10)
        alienBulletCO = (canvas.coords(alienBulletImageItem))

        if (image == 2) and (imageCounter == 10):
            canvas.itemconfig(alienBulletImageItem, image = alienBulletImage2)
            image = 1
            imageCounter = 0 
        elif (image == 1) and (imageCounter == 10):
            canvas.itemconfig(alienBulletImageItem, image = alienBulletImage)
            image = 2
            imageCounter = 0
            
        imageCounter = imageCounter+1
        
        if ((alienBulletCO[1])+ (0.5*alienBulletImage.height())) > canvasHeight:
            bulletFalling=False
